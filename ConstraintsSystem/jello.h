/*
 ...
 Options: "Gravity" key:'g'; "User Force" up:'k', down:'l',left:'<',right:'>'
 "Random Force" key:'r'
 "Damping Force" key:'d'
 */
...
extern int g_vLeftPressPos[2];
extern int g_vDiffPos[2];
extern int g_iCircle[2];
...
extern int Pause, viewingMode, saveScreenToFile, pushUp, pushDown, pushLeft, pushRight;
extern bool isGravity, isDamping, isRandomforce, isRK4, isConstraint, isPlane;
extern double  tempCount;
...
// computes dotproduct of three vectors, which are given as points and specified by floating-point coordinates
// struct point vector1, vector2, double dest
// result goes into dest
...
// compute vector distance in space
// struct point dest
// result goes into length
#define pLENGTH(dest, length)\
\
length = sqrt((dest).x * (dest).x + (dest).y * (dest).y + (dest).z * (dest).z);
